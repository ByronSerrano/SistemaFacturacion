...
public class MenuPrincipal extends JFrame {
	...
	public MenuPrincipal() {
		...
		mntmNewMenuItem_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (listCliente.isVisible() == false) {
				panel.add(listCliente);
				listCliente.setVisible(true);
				Dimension desktopSize = panel.getSize();
		        Dimension FrameSize = listCliente.getSize();
		        listCliente.setLocation((desktopSize.width - FrameSize.width)/2, (desktopSize.height- FrameSize.height)/2);
		        listCliente.show();
				}
			}
		});
